...
namespace HISPlayer
{
    public class HISPlayerUIHelper : MonoBehaviour
    {
        ...
        public Slider[] volumeSlider; //here is the new slider
        ...
        public void InitUI(HISPlayerController controller)
        {
            ...
            for (int i = 0; i < m_controller.totalScreens; i++)
            {
                ...
                //we check the volume slider in the very own loop
                if (volumeSlider != null && i < volumeSlider.Length && volumeSlider[i] != null)
                {
                    int index = i;
                    volumeSlider[i].minValue = 0f;
                    volumeSlider[i].maxValue = 1f;
                    volumeSlider[i].value = 1f;
                    volumeSlider[i].onValueChanged.AddListener((value) =>
                    {
                        m_controller.OnVolumeChanged(index, value);
                    });
                }
            }
        }
